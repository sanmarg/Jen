...
struct scene_reader {
    ...
    template< class T > void read_harness( std::string& field_name, const json& j, harness< T >& h, std::map< std::string, std::function< T ( T&, element_context& ) > >& harness_fns );
    #define READ_ANY_HARNESS( _T_, _U_ ) void read_any_harness( std::string field_name, const json& j, harness< _T_ >& h, std::map< std::string, std::function< _T_ ( _T_&, element_context& ) > >& harness_fns )  { read_harness< _T_ >( field_name, j, h, _U_ ); }
